...
def determine_match_type(row, fuzzy_threshold):
    ...
    elif row.get("match_reason", "").lower().find("sector matched") != -1 and row.get("title_score", 0) > 0:
        return "C - keyword+title"
    elif row.get("match_reason", "").lower().find("inferred") != -1 and row.get("title_score", 0) > 0:
        return "D - inferred+title"
...
def score_leads(df, reference_companies, company_map, settings):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Scoring leads"):
        ...
        domain = str(row.get('Website', '')).lower()
        ...
        if resolved_country:
            ...
            if resolved_country.lower() in penalized_countries:
                ...
                score += 1
                geo_score += 1
                explanation.append(f"Geo boost: {resolved_country.title()}")
        else:
