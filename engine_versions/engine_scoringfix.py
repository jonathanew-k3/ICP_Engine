...
def score_leads(df, reference_companies, company_map, settings):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Scoring leads"):
        ...
        elif row.get("Industry", "") in priority_industries:
            match_source = "priority_industry"
        elif row.get('is_keyword_sector', False):
    ...
    df['is_keyword_sector'] = [True if explanation and "keyword sector match" in exp else False for exp in explanations]
